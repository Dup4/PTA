...
int main()
{
	...
	for(j=1;j<i;j++)
	{
		if(q[p[j].ID].C!=q[p[j-1].ID].C) flag=j+1;
	...
	for(j=1;j<i;j++)
	{
		if(q[p[j].ID].M!=q[p[j-1].ID].M) flag=j+1;
	...
	for(j=1;j<i;j++)
	{
		if(q[p[j].ID].E!=q[p[j-1].ID].E) flag=j+1;
	...
	for(j=1;j<i;j++)
	{
		if(q[p[j].ID].A!=q[p[j-1].ID].A) flag=j+1;
	 ...
	 {
	 	cin>>s;
	 	if(q[s].flag) cout<<q[s].rank_min<<" "<<q[s].c<<endl;
	 	else cout<<"N/A\n";
	 }
}

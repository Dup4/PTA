	...
	for(i=0; i<n; i++){
		...
		for(j=0; j<m; j++){
			...
			for(h=97; h<102; h++){
				...
				else list[j].wrong[h-'a']++;
				if(list[j].wrong[h-'a']>list[j].max&&list[j].wrong[h-'a']) list[j].max=list[j].wrong[h-'a'];  //add
				}
				if(list[j].max>max) max=list[j].max;
			//cout<<count<<endl;
			if(flag){             //remove
		...
		for(i=0;i<m;i++)
		{
			for(j=0;j<5;j++)
			{
				if(list[i].wrong[j]==list[i].max) list[i].flag[j]=1;
			}
		}
		flag=1;
		for(i=0;i<m;i++)

...
bool dfs(int al, int ar, int bl, int br, int cl, int cr, int &rt) {
	...
	if (al > ar || bl > br || cl > cr) return true;
//			cout << al << " " << ar << " " << bl << " " << br << " " << cl << " " << cr << endl;
	if (b[bl] != 0 && c[cr] != 0 && b[bl] != c[cr]) return false;
	for (int i = al; i <= ar; ++i) {
		if (a[i] != 0 && b[bl] != 0 && a[i] != b[bl]) continue;
		if (a[i] != 0 && c[cr] != 0 && a[i] != c[cr]) continue;
...
void bfs(int st) {
	...
	while (!que.empty()) {
		...
		res.push_back(_a[u]);
		if (son[u].fi) que.push(son[u].fi);
		if (son[u].se) que.push(son[u].se);
	} 
...
//	cout << num << endl;
	...
	for (int i = 1; i <= n; ++i) if (cnt[i] == 0) num = i; 
//	cout << num << endl;
	for (auto &arr : {_a, _b, _c}) {
		for (int i = 1; i <= n; ++i) if (arr[i] == 0) arr[i] = num; 
		for (int i = 1; i <= n; ++i) {
			cout << arr[i] << " \n"[i == n];
		}
	}

...
inline bool ok(int x, int y) {
	if (hs.get(x, x + n - 1) == hs.get(y, y + n - 1)) return false;
	int l = 1, r = n, res = 0;
	while (r - l >= 0) {
		int mid = (l + r) >> 1;
	...
	if (res < n && s[x + res] > s[y + res]) return true;
	return false;
}
...
int main() {
	...
	vec.pop_back();
	while (!vec.empty()) {
		node tmp = vec.back(); vec.pop_back();
		if (tmp.val == _vec.back().val) ++_vec.back().num;
		else _vec.push_back(tmp);
	}
	...
	for (int i = 1; i < SZ(_vec); ++i) {
		node now = _vec[i];
		if (res.num > now.num) break;
		else if (ok(res.ix, now.ix)) res = now;
	}
